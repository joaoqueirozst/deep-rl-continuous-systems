...
model.learn(total_timesteps=500_000, callback=error_callback)
model.save("model_ball_beam_DQN")
print("Modelo salvo em model_ball_beam_DQN.zip")
...
plt.title('DQN Ball and Beam: Recompensa vs. Timesteps')
plt.legend()
plt.tight_layout()
...
plt.xlabel('Episódio')
plt.ylabel('Recompensa por episódio')
plt.title('DQN Ball and Beam: Recompensa vs. Episódio')
plt.legend()
plt.tight_layout()
