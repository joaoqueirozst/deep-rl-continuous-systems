...
class InvertedPendulum(gym.Env):
    ...
    DISCRETE_ACTIONS = np.linspace(-10.0, 10.0, 21) # Discrete actions
    ...
    def step(self, action):
        action_force = float(self.DISCRETE_ACTIONS[int(action)])
        action_force = np.clip(action_force, -10.0, 10.0)

        if self.randomActuator:
            action_force += 0.01 * np.random.randn()
        ...
        x4dot = (self.g * np.sin(x3) - np.cos(x3) * (action_force + self.m * self.l * x4**2 * np.sin(x3)) / (self.M + self.m)) / (self.l * (4.0/3.0 - self.m * np.cos(x3)**2 / (self.M + self.m)))
        x2dot = ((action_force + self.m * self.l * x4**2 * np.sin(x3)) / (self.M + self.m) - self.m * self.l * x4dot * np.cos(x3) / (self.M + self.m))
        ...
        if self.render_mode == "human":
            ...
            self.hist_force.append(action_force)
